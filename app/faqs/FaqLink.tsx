...
export const FaqLink: React.FC<FaqLinkProps> = ({ href, icon: Icon, label }) => {
  ...
  const content = (
    <>
      <Icon aria-hidden='true' className='w-5 h-5 text-white group-hover:scale-110 transition-transform duration-200' />
      {label}
      <ExternalLink aria-hidden='true' className='w-4 h-4 text-white/70 group-hover:text-white transition-colors duration-200' />
    </>
  );
